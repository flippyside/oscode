...
// 在 head 后插入 new 节点
void list_add(struct ListHead *new, struct ListHead *head) {
	new->next = head->next;
	new->prev = head;
	head->next->prev = new;
	head->next = new;
...
void keyboardHandle(struct StackFrame *sf) {
	...
	if (dev[STD_IN].value < 0) { // 有进程阻塞在STD_IN。with process blocked
		// TODO: deal with blocked situation
		dev[STD_IN].value++;
		// 找到链表尾部的进程结构体指针 pt
		// pt = (ProcessTable*)((uint32_t)(dev[STD_IN].pcb.prev) - (uint32_t)&(((ProcessTable*)0)->blocked));
		pt = CONTAINER_OF(dev[STD_IN].pcb.prev, ProcessTable, blocked);
...
void sysReadStdIn(struct StackFrame *sf) {
	...
	if(dev[STD_IN].value == 0){
		...
		// 将当前进程插入 dev[STD_IN] 的等待队列
		list_add(&pcb[current].blocked, &dev[STD_IN].pcb);
...
void sysSemWait(struct StackFrame *sf) {
	...
	if(sem[i].state == 1){
		sem[i].value--;
		if(sem[i].value < 0){
			// list_add(&pcb[current].blocked, &sem[i].pcb);
			// putStr("\n block:");
			// putNum(current);
			// putStr("\n");
...
void sysSemPost(struct StackFrame *sf) {
	...
	if(sem[i].value <= 0){
		...
		pt = CONTAINER_OF(sem[i].pcb.prev, ProcessTable, blocked);
...
void sysSVarCreate(struct StackFrame *sf){
	...
	putStr("sysSVarCreate: ");

	for(int i = 0; i < MAX_SHARED_VARS; i++){
		if(sharedVar[i].valid == 0){
			sharedVar[i].id = i;
			sharedVar[i].value =value;
			sharedVar[i].refCount = 1;
			sharedVar[i].valid = 1;
			putNum(i);
...
void sysSVarDestroy(struct StackFrame *sf){
	...
	sharedvar_t *svar = (sharedvar_t *)sf->edx;
	int id = *svar;
	if (id >= 0 && id < MAX_SHARED_VARS && sharedVar[id].valid) {
		sharedVar[id].refCount--;
		if (sharedVar[id].refCount <= 0) {
			sharedVar[id].valid = 0;
		}
		pcb[current].regs.eax = 0;
		return;
	}
...
void sysSVarRead(struct StackFrame *sf) {
	...
	putNum(id);
	if (id >= 0 && id < MAX_SHARED_VARS && sharedVar[id].valid) {
		putNum(sharedVar[id].value);
		pcb[current].regs.eax = sharedVar[id].value;
		return;
	}
	pcb[current].regs.eax = -1;
	return;
...
void sysSVarWrite(struct StackFrame *sf){
	...
	int id = sf->edx;
	int value = (int)sf->ebx;

	if (id >= 0 && id < MAX_SHARED_VARS && sharedVar[id].valid) {
		sharedVar[id].value = value;
		pcb[current].regs.eax = 0;
		return;
	}
	pcb[current].regs.eax = -1;
